...
def perfilview(request):
    try:
        info_personal = InfoPersonal.objects.filter(user=request.user).first()
    except InfoPersonal.DoesNotExist:
        info_personal = None

    if request.method == 'POST':
        form = InfoPersonalForm(request.POST, instance=info_personal)
        if form.is_valid():
            form.save()
            return redirect('perfil')
    else:
        form = InfoPersonalForm(instance=info_personal)
    
    return render(request, 'perfil/perfil.html', {'info_personal': info_personal})
...
def editarPerfil(request):
    ...
    if request.method == "POST":
        ...
        info_personal_form = InfoPersonalForm(request.POST)
        if form.is_valid() and info_personal_form.is_valid():
            ...
            info_personal.user = request.user
            info_personal.save()
            return render(request, 'perfil/perfil.html')
    else:
        form = UserEditForm(initial={'username': usuario.username, 'email': usuario.email, 'first_name': usuario.first_name, 'last_name': usuario.last_name})
        info_personal_form = InfoPersonalForm()
